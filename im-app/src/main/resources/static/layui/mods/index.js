...
layui.define(['layer', 'laytpl', 'form', 'upload', 'util', 'element'], function (exports) {
    ...
    var $ = layui.jquery
        ...
        , util = layui.util
        , device = layui.device()
        , layid = location.hash.replace(/^#test=/, '');
    ...
    var active = {
        tabAdd: function (othis) {
            var id = othis.attr('module-id')
            console.log(treeNodes)

            if (existTab(id)) {
                element.tabChange('test', id);
            } else {
                $.get('/api/auth/getModule?id=' + id).done(function (data) {
                    var content = '<iframe frameborder="0" scrolling="0" style="width:100%;height: 600px;" src="' + data.moduleUrl + '"></iframe>'
                    element.tabAdd('test', {
                        title: data.moduleCtitle,
                        content: content,
                        id: data.id
                    })
                    element.tabChange('test', data.id);
                    treeNodes.push(data.id);
                    element.init()
                })
            }
        }
    ...
    function existTab(module) {
        var $title = $('.layui-tab-title li');
        for(var i=0;i<$title.length;i++){
            if($($title[i]).attr('lay-id')==module){
                return true;
            }
        }
        return false;
